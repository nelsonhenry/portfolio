...
$('.slider').each(function () {
  ...
  $('.slide').click(function () {
    ...
    if (s.next().length > 0) {
      s.next().addClass('current').lazyload()
    } else {
      s.parent().children(':first').addClass('current')
    }
  })
});
...
function reveal() {
  $('.w').each(function () {
    let eT = $(this).offset().top + 40;
    let wB = w.scrollTop() + w.height();
    if (wB > eT) {
      $(this).addClass('on')
    } else {
      $(this).removeClass('on')
...
function mP() {
  let fH = f.outerHeight();
  let hH = h.outerHeight() + 60;
  m.css("padding-top", hH);
  if (w.width() > 768) {
    m.css("margin-bottom", fH)
  } else {
...
function changeCrs() {
  if (w.width() > 768) {
    $('a, .w>*:first-child, .info').hover(() => {
      crs.addClass('crs-hover')
    }, () => {
      crs.removeClass('crs-hover')
    });
    $('.slider').hover(() => {
      crs.addClass('crs-plus')
    }, () => {
      crs.removeClass('crs-plus')
    })
  }
};
